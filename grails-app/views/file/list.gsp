			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="objectKey" title="${message(code: 'file.objectKey.label', default: 'Object Key')}" />
				...
				<g:each in="${fileInstanceList}" status="i" var="fileInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${fileInstance.id}">${fieldValue(bean: fileInstance, field: "fileName")}</g:link></td>
					
						<td>${fieldValue(bean: fileInstance, field: "objectKey")}</td>
